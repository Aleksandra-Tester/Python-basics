...
class TestFuzzyMath:
    ...
    def test_addition_with_negative_result(self):
            assert 'negative number' in fuzzy_math(-4, '+', -2)
    ...
    def test_multiplication_with_medium_result(self):
            assert 'medium-sized' in fuzzy_math(3, '*', 6)

    def test_multiplication_with_large_result(self):
            assert 'really big' in fuzzy_math(4, '*', 20)

    def test_invalid_operator(self):
            #assert 'operator' in fuzzy_math(2,'-', 2) -> will see the test fail and exception message: I don't know how to do math with -.
            with pytest.raises(Exception) as exc_info:
                fuzzy_math(2, '-', 2)
